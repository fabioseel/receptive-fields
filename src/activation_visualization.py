...
def _activation_triggered_average(model:nn.Module, n_batch:int=2048):
    ...
    input_tensor= input_tensor[:, None,:,:,:].expand(-1,output.shape[1],-1,-1,-1)
...
def activation_triggered_average(model:nn.Module, n_batch:int=2048, n_iter:int=1):
...
def get_input_output_shape(model: nn.Sequential):
    ...
    for layer in reversed(model[:-_first]):
        if isinstance(layer, nn.Linear):
            ...
            in_size = (in_size - 1) * layer.stride[0] - 2*layer.padding[0] + layer.kernel_size[0]
...
def backprop_maximization(model:nn.Module, n_iter:int=2048, batch_size=16, reduction=True, smoothened=False):
    num_outputs, input_size = get_input_output_shape(model)
    results = torch.zeros((num_outputs, *input_size))
    for i in tqdm(range(num_outputs)):
        output_signal = torch.zeros(num_outputs)
        output_signal[i] = 1
        results[i] = single_backprop_maximization(model, output_signal, input_size, n_iter, batch_size, reduction, smoothened)
    return results

def single_backprop_maximization(model:nn.Module, output_signal:torch.tensor, input_size: torch.Size, n_iter:int=2048, batch_size=16, reduction=True, smoothened=False):
    model.eval()
    ...
    for i in tqdm(range(n_iter)):
        ...
        if smoothened and i > n_iter/3:
            loss = loss + smooth_loss(input_tensor[...,1:,1:],input_tensor[...,:-1,:-1])\
