...
def create_parser():
    ...
    parser.add_argument(
        "--save_hist", action="store_true", help="save the weights after each epoch"
    )
    parser.add_argument(
        "--num_epochs", type=int, default=20, help="stop after n_epochs epoch"
    )
    parser.add_argument(
        "--early_stop", type=int, default=0, help="stop after n epochs of not improving"
    )
    parser.add_argument(
        "--max_num_batches",
        type=int,
...
def open_experiment(path, train_data=True, test_data=False, batch_size=10):
    ...
    dataset_s = load_dataset(
        log["dataset"],
        train_data,
        test_data,
        log["enable_img_transforms"],
        log["min_resize"],
        log["max_resize"],
        log["add_background"],
        model.in_channels == 1,
...
def setup_dataset_transforms(
    enable_img_transforms: bool,
    min_resize: float,
    max_resize: float,
    add_background: str = "rl",
    ...
    if enable_img_transforms:
        ...
        if add_background == "rl":
            path = osp.join(
                receptive_fields.__path__[0], "../resources/empty-viewport.png"
            )
            bg = PIL.Image.open(path).convert("RGB")
    ...
    if mean != 0 or std != 1:
        if not grayscale:
            mean = torch.tensor(mean).repeat(3)
            std = torch.tensor(std).repeat(3)
...
def load_dataset(
    ...
    enable_img_transforms: bool = False,
    min_resize: float = 1,
    max_resize: float = 1,
    add_background: str = "rl",
    grayscale: bool = False,
    ...
    mean: float = 0,
    std: float = 1,
    data_root=osp.join(receptive_fields.__path__[0], "../data"),
):
    assert train_data or test_data

    if dataset == "stl10":
        transf = setup_dataset_transforms(
            enable_img_transforms,
            min_resize,
            max_resize,
            add_background,
            grayscale,
            img_size if img_size != 96 else None,
            mean=mean,
            std=std,
        )
        if train_data:
            train_set = datasets.STL10(
                root=data_root, split="train", download=True, transform=transf
            )
        if test_data:
            test_set = datasets.STL10(
                root=data_root, split="test", download=True, transform=transf
            )
        ...
        if train_data:
            ...
            imagenette_path = osp.join(
                receptive_fields.__path__[0], "../../../data/imagenette2-160"
            )
            train_set = datasets.ImageFolder(
                osp.join(imagenette_path, "train"), transform=transf
            )
        if test_data:
            test_set = datasets.ImageFolder(
                osp.join(imagenette_path, "val"), transform=transf
            )
    else:
        transf = setup_dataset_transforms(
            enable_img_transforms,
            ...
            add_background,
            grayscale,
            img_size if img_size != 32 else None,
            mean=mean,
            std=std,
        )
        if train_data:
            train_set = datasets.CIFAR10(
                root=data_root, train=True, download=True, transform=transf
            )
        if test_data:
            test_set = datasets.CIFAR10(
                root=data_root, train=False, download=True, transform=transf
            )
    if train_data and test_data:
