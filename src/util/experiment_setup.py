...
def open_experiment(path, train_data=True, test_data=False, batch_size=10):
    ...
    dataset_s = load_dataset(log["dataset"], train_data, test_data, log["enable_img_transforms"], log["min_resize"], log["max_resize"], log["add_background"], model.in_channels==1, model.img_size, log["mean"], log["std"])
...
def setup_dataset_transforms(enable_img_transforms: bool, min_resize: float, max_resize: float, add_background: str="rl", grayscale: bool=False, img_size:int = None, mean=0, std=1):
    transf = []
...
def load_dataset(dataset:str = "cifar10", train_data:bool = True, test_data:bool = False, enable_img_transforms: bool = False, min_resize: float = 1, max_resize: float = 1, add_background: str = "rl", grayscale: bool = False, img_size:int = None, mean:float=0, std:float=1, data_root = "../data"):
    assert train_data or test_data

    if dataset == "stl10":
        transf = setup_dataset_transforms(enable_img_transforms, min_resize, max_resize, add_background, grayscale, img_size if img_size != 96 else None, mean=mean, std=std)
        if train_data:
